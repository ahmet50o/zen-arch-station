...
# Update keyring first to avoid signature errors on older ISOs
pacman -Sy --noconfirm archlinux-keyring

if pacman -S --noconfirm reflector && command -v reflector &>/dev/null; then
    reflector --country "$target_country" --latest 20 --protocol https --sort rate --save /etc/pacman.d/mirrorlist || warn "Reflector failed. Using default mirrorlist."
else
    warn "Reflector not available. Using default mirrorlist."
fi
